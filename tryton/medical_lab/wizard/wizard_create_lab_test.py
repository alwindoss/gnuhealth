...
class CreateTestReport(Wizard):
    ...
    states = {
        'init': {
            'result': {
                'type': 'form',
                'object': 'medical.lab.test.create.init',
                'state': [
    ...
    def _action_open_medical_lab(self, ids):
        model_data_obj = self.pool.get('ir.model.data')
        act_window_obj = self.pool.get('ir.action.act_window')

        act_window_id = model_data_obj.get_id('medical_lab',
                'medical_action_tree_lab')
    ...
    def _create_lab_test(self, data):
        ...
        return self._action_open_medical_lab(lab_id)
