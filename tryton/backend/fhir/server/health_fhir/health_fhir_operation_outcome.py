...
class health_OperationOutcome_Issue(supermod.OperationOutcome_Issue):
    ...
    def set_details(self, details):
        if details:
            ...
            super(subOperationOutcome_Issue, self).set_details(st)

    def add_location(self, location):
        if location:
            st = supermod.string(value=location)
            super(subOperationOutcome_Issue, self).add_location(st)

    def set_severity(self, severity):
        if severity:
            sev=supermod.misc.IssueSeverity(value=severity)
            super(subOperationOutcome_Issue, self).set_severity(sev)
